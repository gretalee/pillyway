...
describe('WaypointsService.findBySlug()', () => {
  ...
  it('returns WaypointDetailDto when the slug matches a CaminoPoint', async () => {
    const pointWithRelations = {
      ...basePoint,
      accommodations: [baseAccommodation],
      sights: [baseSight],
    };
    const prismaMock = {
      caminoPoint: {
        findUnique: vi.fn().mockResolvedValue(pointWithRelations),
      },
    };
    ...
    expect(result.accommodations[0].name).toBe('Albergue Municipal');
    expect(result.sights).toHaveLength(1);
    expect(result.sights[0].name).toBe('Porte Saint-Jacques');
  ...
  it('returns empty accommodations and sights arrays when none exist', async () => {
    const pointWithRelations = {
      ...basePoint,
      accommodations: [],
      sights: [],
...
describe('WaypointsService.createAccommodation()', () => {
  ...
  it('defaults imageUrls to empty array when not provided in DTO', async () => {
    const dtoWithoutUrls: CreateAccommodationDto = { name: 'Hostel Simple' };
    const prismaMock = {
      caminoPoint: {
        findUnique: vi.fn().mockResolvedValue(basePoint),
      },
