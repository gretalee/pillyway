...
test.describe("Language switcher — immediate UI update", () => {
  ...
  test("clicking DE after EN switches back to German strings", async ({
    page,
  }) => {
    // Start in English.
    await setLocaleCookie(page, "en");
  ...
  test("the active language button has a visible distinct state", async ({
    page,
  }) => {
    ...
    await expect(page.getByRole("button", { name: "DE" })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
  });
...
test.describe("Language preference persistence", () => {
  test("language remains English after navigating to another page", async ({
    page,
  }) => {
  ...
  test("pillyway-locale cookie has SameSite=Lax attribute", async ({
    page,
  }) => {
    await setLocaleCookie(page, "de");
    await page.goto("/");
